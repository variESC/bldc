...
//		}
		...
		if (current_mode_rel && cc_button && fabsf(pwr) < 0.001) {
			...
			if (config.multi_esc) {
				float current_filt = mc_interface_get_tot_current_directional_filtered();

				for (int i = 0;i < CAN_STATUS_MSGS_TO_STORE;i++) {
					can_status_msg *msg = comm_can_get_status_msg_index(i);

					if (msg->id >= 0 && UTILS_AGE_S(msg->rx_time) < MAX_CAN_AGE) {
						comm_can_set_current(msg->id, current_filt);
					}
				}
			}
			...
			if (current_mode_brake) { //If braking applied
				...
				if (config.multi_esc) {
					for (int i = 0;i < CAN_STATUS_MSGS_TO_STORE;i++) {
						...
						if (msg->id >= 0 && UTILS_AGE_S(msg->rx_time) < MAX_CAN_AGE) {
							comm_can_set_current_brake(msg->id, fabsf(current));
						}
					}
				}
			} else {
					...
					for (int i = 0;i < CAN_STATUS_MSGS_TO_STORE;i++) {
						...
						if (msg->id >= 0 && UTILS_AGE_S(msg->rx_time) < MAX_CAN_AGE) {
							...
							if (config.tc && !autoTCdisengaged) {
								...
								float diff = rpm_tmp - rpm_lowest;
								decoded_level = utils_map(diff, 0.0, config.tc_max_diff, decoded_level, 0.0);
							}
